...
public class TransactionSignature extends ECKey.ECDSASignature {
     ...
     */
    public TransactionSignature(ECKey.ECDSASignature signature, SigHash mode, boolean anyoneCanPay, byte forkID) {
        super(signature.r, signature.s);
        setSigHash(mode, anyoneCanPay, forkID);
    }
     ...
     */
    public void setSigHash(SigHash mode, boolean anyoneCanPay, byte forkID) {
        sighashFlags = calcSigHashValue(mode, anyoneCanPay, forkID);
    }
     ...
     */
    public static int calcSigHashValue(SigHash mode, boolean anyoneCanPay, byte forkID) {
     ...
     */
    public static TransactionSignature decodeFromBitcoin(byte[] bytes, boolean requireCanonical) throws RuntimeException {
        // Bitcoin encoding is DER signature + sighash byte.
        if (requireCanonical && !isEncodingCanonical(bytes))
            throw new RuntimeException("Signature encoding is not canonical.");
        ECKey.ECDSASignature sig;
        try {
            sig = ECKey.ECDSASignature.decodeFromDER(bytes);
        } catch (IllegalArgumentException e) {
            throw new RuntimeException("Could not decode DER", e);
        }
